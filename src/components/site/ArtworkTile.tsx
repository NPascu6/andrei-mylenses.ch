    ...
    return (
        ...
        >
            <ExpandableImage
                ...
                alt={photo.title}
                imgClassName={`w-full object-cover transition-transform duration-700 group-hover:scale-[1.03] ${imageClassName}`}
                imgStyle={{objectPosition: getPhotoObjectPosition(photo), ...imageStyle}}
                orderDetails={{
                    title: photo.title,
            ...
            >
                ...
                <div className="absolute left-4 top-4 rounded-full border border-white/12 bg-black/42 px-3 py-2 text-[10px] uppercase tracking-[0.24em] text-white/78 backdrop-blur-sm">
                    {badge || (photo.printReady ? copy.artworkTile.collectorPrint : photo.category)}
                </div>
