...
import {
    getPreferredBrowserLocales,
    readLocalStorageItem,
    setDocumentLanguage,
    writeLocalStorageItem,
} from '../utils/browserRuntime';
...
const getInitialLocale = (): AppLocale => {
    ...
    const stored = readLocalStorageItem(localeStorageKey);
    if (stored && isAppLocale(stored)) {
        return stored;
    }

    const browserLocales = getPreferredBrowserLocales();
...
export const I18nProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
    ...
    useEffect(() => {
        writeLocalStorageItem(localeStorageKey, locale);
